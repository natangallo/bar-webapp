document.addEventListener('DOMContentLoaded', function () {
    ...
    let carrello = [];
    let categorieCount = {}; // Deve essere dichiarato all'inizio
        ...
        fetch('prodotti/prodotti.php')
            ...
            .then(prodotti => {
                ...
                prodotti.forEach(prodotto => {
                    if (!categorieCount[prodotto.categoria]) {
                        categorieCount[prodotto.categoria] = 0;
                    }
                    categorieCount[prodotto.categoria]++;
                });

                // Crea e aggiungi gli elementi dei prodotti
                prodotti.forEach(prodotto => {
    ...
    function creaElementoProdotto(prodotto) {
        ...
        const buttonClass = getButtonClass(prodotto.categoria);
        div.innerHTML = `
            <span>${prodotto.nome} - €${prodotto.prezzo}</span>
            <button class="${buttonClass}" data-id="${prodotto.id}" data-prezzo="${prodotto.prezzo}">Aggiungi</button>
        `;
        div.querySelector('button').addEventListener('click', aggiungiAlCarrello);
        return div;
    }

    // Funzione per ottenere la classe del pulsante in base alla categoria
    function getButtonClass(categoria) {
        const count = categorieCount[categoria];
        if (categoria === 'gelati') {
            return count > 2 ? 'category-blue' : 'category-blue-light'; // Adatta il limite secondo le tue esigenze
        } else if (categoria === 'bibite') {
            return count > 2 ? 'category-purple' : 'category-purple-light';
        }
        return ''; // Classe di default se la categoria non è riconosciuta
    }
